    ...
    <div class="p-2">
      <div class="mb-2">
        <h1 class="text-lg font-semibold text-center text-[#5d5d5d]">
          Tổng điểm: {{ totalScore }} / {{ reviewExam.max_score }}
        </h1>
        <h1 class="text-lg font-semibold text-center text-[#5d5d5d]">
          Số câu đúng: {{ successQuestions }} / {{ totalQuestions }}
        </h1>
        <p></p>
      </div>
...
export default {
  ...
  computed: {
    ...
    totalQuestions() {
      return this.reviewExam && this.reviewExam.questions
        ? this.reviewExam.questions.length
        : 0
    },
  },
  mounted() {
      ...
      this.addCommentExam(payload)
        .then(() => {
          ...
          setTimeout(() => {
            ...
            // const response = this.getReviewExam(this.idExam)
